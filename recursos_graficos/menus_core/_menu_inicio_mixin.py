...
class MenuInicioMixin:
    ...
    def Menu_inicio(self):
        ...
        proporcion = self.logo_rummy.get_height() / self.logo_rummy.get_width()
        nuevo_ancho = max_ancho
        nuevo_alto = int(max_ancho * proporcion)

        logo_escalado = pygame.transform.smoothscale(self.logo_rummy, (nuevo_ancho, nuevo_alto))

        x_centro = (constantes.ANCHO_MENU_I - nuevo_ancho) // 2

        posicion_logo = (
        x_centro - 200,  # mueve izquierda/derecha
        int(constantes.ALTO_MENU_I * 0.15)  # mueve arriba/abajo
        )

        menu_inicio.agregar_imagen(logo_escalado, posicion_logo, 1)
    ...
    def crear_elementos_control_inicio(self, menu_inicio):
        ...
        espacio = 140  # separación entre botones
        y_inicial = 100  # sube o baja todo el grupo
